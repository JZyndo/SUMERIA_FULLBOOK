...
public class Stereo3D : MonoBehaviour {
	...
	public void SetDefault () {
        ...
        Camera leftCamera = _leftCam.GetComponent<Camera>();
        Camera rightCamera = _rightCam.GetComponent<Camera>();

        leftCamera.renderingPath = camera.renderingPath;
        rightCamera.renderingPath = camera.renderingPath;
	}
	...
	public void SetTargetTextures (RenderTexture l, RenderTexture r) {
        ...
        leftCamera.targetTexture = _leftCamRT;
        rightCamera.targetTexture = _rightCamRT;
	...
	void SetInterlace () {
        ...
        leftCamera.depth = camera.depth - 2;
        rightCamera.depth = camera.depth - 1;
	...
	void SetShutter () {
		...
		leftCamera.depth = camera.depth + 1;
		rightCamera.depth = camera.depth + 1;
		leftCamera.enabled = true;
	...
	void SetSideBySide (bool isReversed) {
        ...
        {
            leftCamera.rect = new Rect(0.5f, 0, 0.5f, 1);
            rightCamera.rect = new Rect(0f, 0, 0.5f, 1);
        }
    ...
    void SetTopBottom(bool isReversed)
    {
        ...
        if (isReversed)
        {
            leftCamera.rect = new Rect(0, 0, 1f, 0.5f);
            rightCamera.rect = new Rect(0f, 0.5f, 1f, 0.5f);
        }
        else
        {
            leftCamera.rect = new Rect(0f, 0.5f, 1f, 0.5f);
            rightCamera.rect = new Rect(0, 0, 1f, 0.5f);
        }
	...
	void UpdateView () 
    {
        ...
        leftCamera.projectionMatrix = _cameraBody.NodalCamera.projectionMatrix;
        rightCamera.projectionMatrix = _cameraBody.NodalCamera.projectionMatrix;
