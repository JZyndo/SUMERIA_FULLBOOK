...
public class PsdLayerExtractor
{
        ...
        string info, System.Action<PsdLayerExtractor> whenPsdFileChanged)
    {
		...
		*/
        ...
        //set the resolution of the psd file
        Texture2D psd_tex = (Texture2D)this.PsdFileAsset;


        this.Psd = new PsdParser.PSD();
        this.Psd.loadHeader(filePath);

        var w = this.Psd.headerInfo.width;
        var h = this.Psd.headerInfo.height;

        resolution = new Vector4(w, h);
    ...
    {
        this.RootGameObject = GameObject.FindWithTag("Main");
    ...
    public Texture MakeSlicedSprites(ref byte[] data, Layer layer, PsdLayerRect area)
    {
    ...
    public Texture MakeTexture(ref byte[] data, Layer layer)
    {
        var channelCount = layer.psdLayer.channels.Length;
        var pitch = layer.psdLayer.pitch;
        var w = layer.psdLayer.area.width;
